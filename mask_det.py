...
test_image=image.load_img('dataset/without_mask/augmented_image_1.jpg',target_size=(150,150,3))
# test_image=image.load_img(dataset/with_mask/21.jpg',target_size=(150,150,3))

test_image
test_image=image.img_to_array(test_image)
test_image=np.expand_dims(test_image,axis=0)
type(model_saved.predict_classes(test_image)[0][0])
...
while cap.isOpened():
    ...
    for (x, y, w, h) in face:
        ...
        test_image = image.img_to_array(test_image)
        test_image = np.expand_dims(test_image, axis=0)
        pred = model_saved.predict_classes(test_image)[0][0]
        if pred == 1:
